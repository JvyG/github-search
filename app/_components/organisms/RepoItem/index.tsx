...
const formateDate = (date: string = '') => {
  ...
  if (secondsDiff < 86400) {
    ...
    if (minutes < 3600) return `Updated ${minutes} minute${minutes > 1 ? 's' : ''} ago`
      ...
      <div key={name} className="flex flex-row gap-[12px]">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex gap-[8px]">
            {topics?.slice(0, windowSize.width < breakpoints.sm ? 2 : 4).map(topic => (
              <Chip key={topic} label={topic} className="mb-[4px]"/>
            ))}
          </div>
